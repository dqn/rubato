...
use beatoraja_types::player_information::PlayerInformation;
use beatoraja_types::score_data_cache::ScoreDataCache;
                            ...
                            match prov.fetch_rival_scores(&rival_info_clone) {
                                Ok(scores) => {
                                    if let Ok(scoredb) = ScoreDatabaseAccessor::new(&db_path_clone)
                                    {
                                        ...
                                        let refs: Vec<&beatoraja_types::score_data::ScoreData> =
                                            scores.iter().collect();

...
impl WebhookHandler {
    ...
    pub fn create_webhook_payload(
        ...
        let webhook_name = current_state.resource.get_config().get_webhook_name();
        payload.insert(
            "username".to_string(),
        ...
        let webhook_avatar = current_state.resource.get_config().get_webhook_avatar();
        payload.insert(
        ...
        if current_state.resource.get_config().get_webhook_option() == 2 {
    ...
    fn get_bp_count(score: &ScoreData) -> i32 {
        score.get_judge_count(3) + score.get_judge_count(4) + score.get_judge_count(5)
    }
    ...
    fn format_random(rd: &ReplayData) -> String {
        ...
        match rd.randomoption {
            ...
            2 | 3 => {
                // RANDOM, R-RAN
                if !rd.lane_shuffle_pattern.is_empty() {
                    for i in 0..7.min(rd.lane_shuffle_pattern[0].len()) {
                        sb += &format!("{}", rd.lane_shuffle_pattern[0][i] + 1);
                    }
                }
            }
            4 => {

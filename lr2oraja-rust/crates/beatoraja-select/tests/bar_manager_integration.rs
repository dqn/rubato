...
use beatoraja_select::bar::search_word_bar::SearchWordBar;
use beatoraja_select::bar::song_bar::SongBar;
use beatoraja_select::bar_manager::{BarManager, UpdateBarContext};
use beatoraja_select::bar_sorter::BarSorter;
use beatoraja_select::null_song_database_accessor::NullSongDatabaseAccessor;
...
fn update_bar_root_clears_directory_stack() {
    ...
    manager.dir.push(Box::new(Bar::Folder(Box::new(
        beatoraja_select::bar::folder_bar::FolderBar::new(None, "test_dir".to_string()),
    ))));
...
fn close_returns_to_parent() {
    ...
    manager.dir.push(Box::new(Bar::Folder(Box::new(
        beatoraja_select::bar::folder_bar::FolderBar::new(None, "parent".to_string()),
    ))));

...
| 18e-9 | BMSPlayerInputProcessor replacement: beatoraja-result stub → `pub use` from beatoraja-input, `reset_scroll()` uncommented. beatoraja-select: BMSPlayerInputProcessor/ControlKeys/KeyCommand stubs → real types, `&` → `&mut` caller update. IRStatus/IRSendStatusMain moved from stubs.rs to own modules |
...
- **Remaining stubs.rs:** lifecycle stubs, cross-crate re-exports, skin/rendering types. beatoraja-result: MainController (6 methods), SkinObjectData (minimal, real type incompatible — 20+ fields). IRStatus/IRSendStatusMain moved to own modules (18e-9, send() still stubbed). beatoraja-modmenu: Skin/SkinObject/Rectangle, MainState, MainController, MusicSelector/Bar/SongBar (RandomTrainer/SongManagerMenu delegated to beatoraja-types in 18e-8). beatoraja-select: MainState, RandomCourseData, RankingData, skin rendering stubs, EventType, AudioDriver, TimerState, DownloadTask (SongManagerMenu delegated in 18e-8, BMSPlayerMode/PlayerInformation replaced in 18e-7, BMSPlayerInputProcessor/ControlKeys/KeyCommand replaced in 18e-9). **beatoraja-pattern: stubs.rs deleted (18e-8), zero remaining stubs.** Clipboard → arboard (18e-6). freq_trainer → beatoraja-modmenu re-export (18e-6)

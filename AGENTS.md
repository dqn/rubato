...
| 18e-2 | Lifecycle stub replacement (partial): MainController stubs removed from 5 crates (obs: `state_type()` on MainState trait; external: `NullMainController`; decide: unused trait impl removed; select: dead code deleted, `MainState` simplified to empty marker trait; modmenu: dead code removed + stub PlayerConfig/SkinConfig/SkinConfigProperty replaced with real beatoraja-types, ~15 call sites updated for `Option<>` handling). beatoraja-ir had no stub. 2 MainController crates blocked: result, md-processor. PlayerResource stubs replaced in 3 crates (select: empty struct deleted; decide: `Box<dyn PlayerResourceAccess>` from beatoraja-types, `PlayerResourceRef`/`PlayerConfigRef` removed; external: `Box<dyn PlayerResourceAccess>` wrapper + `original_mode: Mode` from bms-model, stub Mode replaced with real enum, callers updated for `Option<>` returns). 1 PlayerResource crate blocked: result (mutable access + type gaps). Dead `src/pending/` files deleted from golden-master. 1241 tests pass |
...
- **Lifecycle:** MainController stubs: result (6 methods actively used, blocked), md-processor (dead code, deferred). modmenu retains 3-method MainController stub (get_config, get_player_config, save_config) until real MainController exists. PlayerResource stubs: result (blocked by mutable access + type gaps)
...
- **PlayerResourceAccess trait gap:** Trait is read-only (29 methods), but beatoraja-result needs 5 mutable getters (`get_score_data_mut`, `get_replay_data_mut`, `get_course_score_data_mut`, `get_course_replay_mut`, `get_course_gauge_mut`). Also needs methods returning types from bms-model/beatoraja-core (`get_bms_model`, `get_play_mode`, `get_ranking_data`, `get_original_mode`) which can't be added to beatoraja-types without introducing circular deps. Stub uses incompatible types (`FloatArray` vs `Vec<f32>`, `GrooveGaugeStub` vs `GrooveGauge`). Solution: per-crate extension trait or expand `PlayerResourceAccess` with mutable variants + move return types to beatoraja-types.

...
| 18e-10 | SkinHeader stub → real `pub use beatoraja_skin::skin_header::SkinHeader` in beatoraja-select. NullSongDatabaseAccessor moved from stubs.rs to own module. RandomCourseData/RandomStageData → real types from beatoraja-core (getters added). **All actionable stub replacements exhausted** — 3 rounds of full audit confirm remaining ~2,600 lines blocked by rendering/IR/database/per-screen implementations |
...
- **Remaining stubs.rs (3 rounds audited — all actionable replacements exhausted):** 16 files, ~2,600 lines total. beatoraja-result: MainController (6 methods), SkinObjectData. beatoraja-select: MainState, RankingData/RankingDataCache (constant value mismatch: stub ACCESS=0 vs real ACCESS=1), SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering), EventType/AudioDriver (in use), DownloadTask (in use). RandomCourseData/RandomStageData replaced (18e-10), SkinHeader replaced (18e-10), NullSongDatabaseAccessor moved to own module (18e-10). beatoraja-modmenu: Skin/SkinObject/Rectangle, MainState, MainController, MusicSelector/Bar/SongBar (API mismatch: stub MusicSelector is zero-size, real is large struct). beatoraja-decide: MainControllerRef + InputProcessor/AudioProcessor stubs. beatoraja-external: LibGDX stubs (Pixmap/GdxGraphics/BufferUtils/PixmapIO), Twitter4j (intentional bail), Property stubs (MainState type mismatch), TableData/TableFolder (API mismatch: String vs Option<String>). **beatoraja-obs/stream/ir/md-processor: clean** (re-exports only, no real stubs). **beatoraja-pattern: zero stubs (18e-8).** All remaining stubs blocked by: rendering pipeline, IR/network layer, database queries, API type mismatches, or per-screen MainState implementations

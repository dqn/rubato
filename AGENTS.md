...
| 18e-9 | BMSPlayerInputProcessor replacement in beatoraja-result: stub → `pub use` from beatoraja-input, `reset_scroll()` uncommented, MainController creates real instance via Config/PlayerConfig defaults |
...
- **Remaining stubs.rs:** lifecycle stubs, cross-crate re-exports, skin/rendering types. beatoraja-result: MainController, IRStatus/IRSendStatusMain (real types in stubs.rs, send() stubbed), SkinObjectData (minimal, real type incompatible — 20+ fields). BMSPlayerInputProcessor resolved (18e-9). beatoraja-modmenu: Skin/SkinObject/Rectangle, MainState, MainController, MusicSelector/Bar/SongBar (RandomTrainer/SongManagerMenu delegated to beatoraja-types in 18e-8). beatoraja-select: MainState, RandomCourseData, RankingData, BMSPlayerInputProcessor (needs `&self`→`&mut self` caller chain update), skin rendering stubs, EventType, AudioDriver, TimerState, DownloadTask (SongManagerMenu delegated in 18e-8, BMSPlayerMode/PlayerInformation replaced in 18e-7). **beatoraja-pattern: stubs.rs deleted (18e-8), zero remaining stubs.** Clipboard → arboard (18e-6). freq_trainer → beatoraja-modmenu re-export (18e-6)

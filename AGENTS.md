...
| 18e-9 | BMSPlayerInputProcessor replacement: beatoraja-result stub → `pub use` from beatoraja-input, `reset_scroll()` uncommented. beatoraja-select: BMSPlayerInputProcessor/ControlKeys/KeyCommand stubs → real types, `&` → `&mut` caller update. IRStatus/IRSendStatusMain moved to own modules. Dead code cleanup (unused imports, TimerState). Full stub audit: 16 files, ~2,700 lines — 97% correctly stubbed, no further easy wins |
| 18e-10 | SkinHeader stub → real `pub use beatoraja_skin::skin_header::SkinHeader` in beatoraja-select. NullSongDatabaseAccessor moved from stubs.rs to own module. RandomCourseData/RandomStageData → real types from beatoraja-core (getters added) |
| 18e-11 | Cross-crate stub replacements: RankingData/RankingDataCache → real beatoraja-ir types (fixes latent off-by-1 constant bug), InputProcessorStub/ControlKeysStub → real beatoraja-input types (Box::leak for &mut, borrow-conflict fix), TableData/TableFolder/TableDataAccessor/TableAccessor → real beatoraja-core types (&TableData→&mut TableData). ~160 lines removed. **4 rounds of full audit — all actionable replacements exhausted**, remaining ~2,440 lines blocked by rendering/IR/database/per-screen implementations |
...
- **MainController:** result (6 methods actively used, PlayDataAccessor resolved in 18e-4, BMSPlayerMode resolved in 18e-5, BMSPlayerInputProcessor resolved in 18e-9, remaining blocker: IRConnection not implemented), md-processor (intentional adapter, deferred), modmenu (3-method stub: get_config, get_player_config, save_config — until real MainController exists)
- **Remaining stubs.rs (4 rounds audited — all actionable replacements exhausted):** 16 files, ~2,440 lines total. beatoraja-result: MainController (6 methods), SkinObjectData. beatoraja-select: MainState, SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering), EventType/AudioDriver (in use), DownloadTask (in use). RankingData/RankingDataCache replaced (18e-11). beatoraja-modmenu: Skin/SkinObject/Rectangle, MainState, MainController, MusicSelector/Bar/SongBar (API mismatch). beatoraja-decide: MainControllerRef (change_state/get_audio_processor), AudioProcessorStub, SkinStub. InputProcessorStub/ControlKeysStub replaced (18e-11). beatoraja-external: LibGDX stubs (Pixmap/GdxGraphics/BufferUtils/PixmapIO), Twitter4j (intentional bail), Property stubs (MainState type mismatch), ScoreDatabaseAccessor, MainState/MainStateListener/AbstractResult/ScreenType. TableData/TableFolder/TableDataAccessor/TableAccessor replaced (18e-11). **beatoraja-obs/stream/ir/md-processor: clean** (re-exports only, no real stubs). **beatoraja-pattern: zero stubs (18e-8).** All remaining stubs blocked by: rendering pipeline, IR/network layer, database queries, or per-screen MainState implementations
...
- **&self→&mut self borrow conflict in composed structs:** When replacing InputProcessorStub (`&self`) with real BMSPlayerInputProcessor (`&mut self` on `is_control_key_pressed`), callers like `self.main.get_input_processor()` + `self.main.get_audio_processor()` create overlapping `&mut` borrows. Solution: scoped block pattern — collect all input results into local booleans first, then use them after the mutable borrow ends. `let (decide, cancel) = { let input = self.main.get_input_processor(); ... (decide, cancel) };`
- **Latent constant value bugs in stubs:** RankingData stub had ACCESS=0/FINISH=1/FAIL=2 (off by 1 from Java NONE=0/ACCESS=1/FINISH=2/FAIL=3). No callers used constants by name, so bug was dormant. Stub replacement silently fixes such bugs. Lesson: audit stub constants against Java source even when they appear correct.

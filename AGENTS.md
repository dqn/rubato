...
- **Remaining stubs.rs (audited — no further easy wins):** 16 files, ~2,700 lines total. beatoraja-result: MainController (6 methods), SkinObjectData. beatoraja-select: MainState, RandomCourseData/RankingData (blocked: IR), SkinHeader/SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering), EventType/AudioDriver (in use), NullSongDatabaseAccessor/DownloadTask (in use). beatoraja-modmenu: Skin/SkinObject/Rectangle, MainState, MainController, MusicSelector/Bar/SongBar. beatoraja-decide: MainControllerRef + InputProcessor/AudioProcessor stubs. beatoraja-external: LibGDX stubs (Pixmap/GdxGraphics/BufferUtils/PixmapIO, used by screenshot exporters), Twitter4j (intentional bail), Property stubs. **beatoraja-pattern: zero stubs (18e-8).** All remaining stubs blocked by: rendering pipeline, IR/network layer, database queries, or per-screen MainState implementations

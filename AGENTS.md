...
All phases complete. 1241 tests pass. Zero runtime `todo!()`/`unimplemented!()`. Phase 16b partially done (2 duplicate pending tests deleted; 9 e2e tests activated; 2 pending tests activated by 18d; 1 pending test activated by programmatic rewrite; 1 remaining blocked by SkinData→Skin pipeline + Lua loader; dead pending src files deleted). Phase 18a (core judge loop) complete. Phase 18b (rendering state providers) complete. Phase 18c (audio decode API) complete. Phase 18d (BGA/skin test APIs) complete. Phase 18e-1 (cross-crate stub deduplication) complete. Phase 18e-2 (lifecycle stub replacement) partially complete — obs/external/decide/ir/select/modmenu MainController done (7 of 8), md-processor reclassified as intentional adapter; select/decide/external PlayerResource done; PlayerResourceAccess trait expanded with 3 mutable getters; result MainController 3 unused methods pruned; 1 MainController + 1 PlayerResource crate remaining (result: blocked on type gaps). Phase 18f (e2e test activation) complete. Phase 18g (BRD replay codec) complete.
...
| 16a–c | Tests: 715 unit + 121 golden master + 32 integration (compare_rule + compare_pattern reactivated with Java LCG fix). 16b partial: 2 duplicate pending tests deleted; 9 e2e tests activated (Phase 18f); 2 pending tests activated by 18d; 1 pending test rewritten with programmatic skin construction (compare_eval_test_skins); 1 remaining blocked (compare_render_snapshot: SkinData→Skin pipeline + Lua loader); `compare_judge_manager.rs` + `compare_audio.rs` activated |
...
| 18e-2 | Lifecycle stub replacement (partial): MainController stubs removed from 5 crates (obs: `state_type()` on MainState trait; external: `NullMainController`; decide: unused trait impl removed; select: dead code deleted, `MainState` simplified to empty marker trait; modmenu: dead code removed + stub PlayerConfig/SkinConfig/SkinConfigProperty replaced with real beatoraja-types, ~15 call sites updated for `Option<>` handling). beatoraja-ir had no stub. md-processor reclassified as intentional adapter pattern (1-method `MainControllerRef` trait). beatoraja-result: 3 unused MainController methods pruned (get_config, get_player_config, change_state); stub type conversion done (FloatArray→Vec<f32>, GdxArray→Vec, GrooveGaugeStub→real GrooveGauge from beatoraja-types); callers updated in music_result.rs/course_result.rs/skin_gauge_graph_object.rs for Option<> handling on get_gauge()/get_groove_gauge(). `PlayerResourceAccess` trait expanded with 3 mutable getters (`get_score_data_mut`, `get_course_replay_mut`, `get_course_gauge_mut`). `NullPlayerResource` changed from unit struct to `#[derive(Default)]` with fields. beatoraja-core `PlayerResource` impl updated. PlayerResource stubs replaced in 3 crates (select: empty struct deleted; decide: `Box<dyn PlayerResourceAccess>` from beatoraja-types; external: `Box<dyn PlayerResourceAccess>` wrapper + `original_mode: Mode`). 1 PlayerResource crate remaining: result (stub types resolved; blocked on wrapper struct for non-trait types: BMSModel/BMSPlayerMode/RankingData). Dead `src/pending/` files deleted from golden-master. 1241 tests pass |
...
- ~~**MainController (obs/external/ir/select/modmenu):**~~ → resolved: obs uses `state_type()`, external uses `NullMainController`, ir had no stub, select dead code deleted, modmenu dead code removed + skin config types replaced with real beatoraja-types (Phase 18e-2)
- ~~**PlayerResource (select/decide/external):**~~ → resolved: select empty struct deleted, decide replaced with `Box<dyn PlayerResourceAccess>`, external replaced with `Box<dyn PlayerResourceAccess>` wrapper + `original_mode: Mode` from bms-model (Phase 18e-2)
- **Lifecycle:** MainController stubs: result (6 methods actively used, 3 unused pruned, blocked on type mismatches), md-processor (intentional adapter pattern, deferred). modmenu retains 3-method MainController stub (get_config, get_player_config, save_config) until real MainController exists. PlayerResource stubs: result (stub type conversion done — FloatArray/GdxArray/GrooveGaugeStub removed, gauge methods match trait signatures; blocked on wrapper struct for non-trait types: BMSModel, BMSPlayerMode, RankingData)
- **Remaining stubs.rs:** lifecycle stubs, cross-crate re-exports, skin/rendering types (modmenu has SkinHeader/Skin/SkinObject/SkinLoader stubs; SkinConfig stubs resolved). `rendering_stubs.rs` in beatoraja-skin resolved (re-exports real beatoraja-render types)
...
- **Programmatic skin construction for tests:** When JSON skin loading is blocked by SkinData→Skin pipeline gap, construct `Skin` objects programmatically: `Skin::new(SkinHeader::default())`, `SkinImage::new_with_single(TextureRegion::new())`, `SkinObject::Image(image)`, `obj.data_mut().set_destination_with_int_timer_ops(...)`, `skin.add(obj)`. Draw conditions set via `op` parameter on first `set_destination_with_int_timer_ops` call only (splits into `dstdraw`/`dstop` via `set_draw_condition_from_ops`). Used for `compare_eval_test_skins.rs` rewrite.
...
- **Lifecycle stub replacement strategy:** Java `MainController.getStateType(MainState)` uses `instanceof`; Rust equivalent is `state_type()` method on `MainState` trait (each concrete state overrides). For crates where `MainController` is stored as a field but never accessed, replace with `NullMainController` from beatoraja-types. For crates with crate-specific methods (get_input_processor, get_play_data_accessor, etc.), stubs must remain until real MainController exposes these APIs or extension traits are created. Key blocker pattern: stubs returning owned values (Config, PlayerConfig) vs trait requiring references.
- **PlayerResourceAccess trait gap (resolved for gauge types):** Trait expanded from 29 to 32 methods: added `get_score_data_mut`, `get_course_replay_mut`, `get_course_gauge_mut`. `NullPlayerResource` changed from unit struct to `#[derive(Default)]` with `course_replay: Vec<ReplayData>` and `course_gauge: Vec<Vec<Vec<f32>>>` fields (needed for `&mut` return from null impl). Stub type conversion done: `FloatArray`→`Vec<f32>`, `GdxArray<T>`→`Vec<T>`, `GrooveGaugeStub`→real `GrooveGauge` from beatoraja-types. Stub `get_gauge()` now returns `Option<&Vec<Vec<f32>>>`, `get_groove_gauge()` returns `Option<&GrooveGauge>` (matching trait). Callers updated for `Option<>` with `.map()/.and_then()/.unwrap_or()`. `IntArray` retained (only in skin_gauge_graph_object.rs graph rendering). Remaining gap: methods returning types from bms-model/beatoraja-core (`get_bms_model`, `get_play_mode`, `get_ranking_data`) can't be on trait due to circular deps. Solution: wrapper struct pattern (like beatoraja-external).
- **PlayerResource stub→trait migration pattern:** For crates with only trait-compatible methods (decide: `set_org_gauge_option`, `get_player_config().gauge`), direct swap to `Box<dyn PlayerResourceAccess>` works cleanly. For crates with direct getters returning non-optional concrete types (external: `get_songdata() -> &SongData` vs trait `Option<&SongData>`), callers must be updated to handle `Option<>`. For crate-local methods not on trait (`get_original_mode()` returning `Mode` from bms-model), use a wrapper struct holding `Box<dyn PlayerResourceAccess>` + the extra field, with delegating methods. No extension trait needed when the extra method is only used within one crate.
- **Empty marker trait for dead MainState:** When a crate has `MainState` trait with `get_main() -> &MainController` but neither method is ever called, removing `get_main()` makes it an empty marker trait. This is safe because all callers use `&dyn MainState` parameters prefixed with `_` (unused). Skin object stubs (SkinText, SkinNumber, SkinImage) accept `&dyn MainState` in their signatures for API compatibility with Java's `MainState` parameter passing.
- **Modmenu skin config stub→real type migration:** Stub `SkinConfig` had `path: String` / `properties: SkinConfigProperty`; real has `path: Option<String>` / `properties: Option<SkinProperty>`. Sub-types similarly wrapped: `SkinOption.name: Option<String>`, `SkinFilePath.name/path: Option<String>`, `SkinOffset.name: Option<String>`. Key patterns: (1) Construction: wrap values in `Some()` — `SkinConfig { path: Some(s), properties: Some(p) }`. (2) Comparison: use `is_some_and()` — `config.get_path().is_some_and(|p| p == skin_path)`. (3) Property iteration: `props.option.iter().flatten()` to skip None entries. (4) Field access: `if let Some(ref name) = option.name { ... }`. (5) `PlayerConfig.skin` changed from `Vec<SkinConfig>` to `Vec<Option<SkinConfig>>` — use `pc.skin[id] = Some(config)` for writes, `if let Some(ref c) = pc.skin[id]` for reads. (6) `PlayerConfig::get_skin(&mut self)` requires mutable borrow; use direct field access `pc.skin` when only reading. (7) `get_play_config` signature changed from `&Mode` to `Mode` (by value) — add `.clone()` at call site. (8) `read_all_player_id` is a free function in real type, not an associated method — import and call directly.

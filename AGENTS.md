...
| 18e-2 | Lifecycle stub replacement (partial): MainController stubs removed from 4 crates (obs: `state_type()` on MainState trait; external: `NullMainController`; decide: unused trait impl removed; select: dead code deleted, `MainState` simplified to empty marker trait). beatoraja-ir had no stub. 3 MainController crates blocked: result, modmenu, md-processor. PlayerResource stubs replaced in 2 crates (select: empty struct deleted; decide: `Box<dyn PlayerResourceAccess>` from beatoraja-types, `PlayerResourceRef`/`PlayerConfigRef` removed). 2 PlayerResource crates blocked: external (caller type mismatches), result (mutable access + type gaps). Dead `src/pending/` files deleted from golden-master. 1241 tests pass |
...
- ~~**MainController (obs/external/ir/select):**~~ → resolved: obs uses `state_type()`, external uses `NullMainController`, ir had no stub, select dead code deleted (Phase 18e-2)
- ~~**PlayerResource (select/decide):**~~ → resolved: select empty struct deleted, decide replaced with `Box<dyn PlayerResourceAccess>` (Phase 18e-2)
- **Lifecycle:** MainController stubs: result (6 methods actively used, blocked), modmenu (type incompatibility, moderate refactoring), md-processor (dead code, deferred). PlayerResource stubs: external (caller type mismatches + missing `get_original_mode()`), result (blocked by mutable access + type gaps)

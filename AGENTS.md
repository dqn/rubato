...
- **Config propagation via command queue:** When menus (SkinMenu, MiscSettingMenu, etc.) save config changes, they must push changes back to MainController via `MainControllerCommandQueue` variants (e.g., `UpdateSkinConfig`, `UpdateSkinHistory`). Writing only to the local `PlayerConfig` clone loses changes between sessions.

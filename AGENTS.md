...
| 18e-2 | Lifecycle stub replacement (partial): MainController stubs removed from 3 crates (obs: `state_type()` added to MainState trait; external: replaced with `NullMainController`; decide: unused trait impl removed). beatoraja-ir had no stub. 4 crates blocked: select (6 crate-specific methods), result (9 actively used methods), modmenu (type incompatibility), md-processor (signature mismatch). PlayerResource stubs remain in 4 crates. 1229 tests pass |

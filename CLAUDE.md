...
- **Skin Object Rendering (Loader Wiring)** — レンダリングインフラ（マルチエンティティ基盤、プロシージャルテクスチャパイプライン、draw モジュール）は実装済みだが、一部ローダーが新フィールドにデータを入れないため描画されない。以下の順序で実装すること:
  ...
  - ~~**addJudgeDetail**~~ — ✅ 完了。`add_judge_detail()` が `dst_judge()` から per-player で呼び出され、4つの独立スキンオブジェクト（SkinImage×2 + SkinNumber×2）を生成。埋め込みテクスチャ `judgedetail.png` を `EMBEDDED_JUDGEDETAIL` ハンドル（`0xFFF0`）で管理。`embedded_textures::load_embedded_textures()` が TextureMap に登録。(`bms-skin/src/loader/lr2_play_loader.rs`, `bms-skin/src/image_handle.rs`, `bms-render/src/embedded_textures.rs`)
- ~~**SkinBar Rendering**~~ — ✅ 完了。LR2 select loader: SRC/DST コマンド完全実装（body/lamp/trophy/level/label/title + LAMPG マッピング）。`BarScrollState`/`BarSlotData` でスクロール状態管理。`skin_renderer.rs` の MultiEntity として `spawn_bar_children()` で60スロットのバーリスト描画。BarManager → SharedGameState → SkinRenderState の状態同期パイプライン。MusicSelectSkinConfig に `center_bar`/`clickable_bar` 追加。(`bms-skin/src/loader/lr2_select_loader.rs`, `bms-render/src/draw/bar.rs`, `bms-render/src/skin_renderer.rs`, `brs/src/state/select/select_skin_state.rs`, `brs/src/state/select/mod.rs`)
- **SongInformation Display** — bms-render 側で未使用。SkinBar とは独立した実装が必要。
- **Preview Music Playback** — ✅ 完了。`PreviewMusicProcessor` が Kira `AudioManager` を使い選曲画面でプレビュー再生。カーソル移動から 400ms 後にプレビュー開始、`SongPreview::Loop`/`Once`/`None` 対応。デフォルト選曲BGM（`bgm/select.*`）のループ再生＋プレビュー終了時の自動復帰。`StateContext.preview_music` 経由で `MusicSelectState` のライフサイクル（create/render/input/shutdown）に統合。(`brs/src/preview_music.rs`, `brs/src/state/select/mod.rs`, `brs/src/main.rs`)

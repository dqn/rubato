...
fn from_config_creates_valid_state() {
    ...
    let jp = crate::play::judge_property::lr2();
...
fn autoplay_judges_all_notes_as_pgreat() {
    ...
    let gp = crate::play::gauge_property::GaugeProperty::Lr2;
...
fn make_manual_ln_jm_with_judge_property(
    ...
    end_us: i64,
    ln_type: LnType,
    judge_property: &crate::play::judge::property::JudgeProperty,
) -> (JudgeManager, Vec<JudgeNote>, GrooveGauge, usize, usize) {
    let model = make_model_with_ln_pair(start_us, end_us);
    let notes = build_judge_notes(&model);
...
fn deferred_ln_release_resolves_exactly_at_margin_boundary() {
    ...
    let mut jp = crate::play::judge_property::lr2();

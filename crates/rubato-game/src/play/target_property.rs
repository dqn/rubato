...
impl InternetRankingTargetProperty {
    ...
    pub fn initiate_load(
        &mut self,
        connection: std::sync::Arc<dyn crate::ir::ir_connection::IRConnection + Send + Sync>,
        chart: crate::ir::ir_chart_data::IRChartData,
        ...
        let handle = std::thread::spawn(move || {
            ...
            if ranking.state() == crate::ir::ranking_data::FINISH {
    ...
    fn target(&mut self, main: &MainController) -> ScoreData {
        ...
        let ranking_data = (|| -> Option<crate::ir::ranking_data::RankingData> {
            ...
            any.downcast::<crate::ir::ranking_data::RankingData>()
                .ok()
                .map(|ranking| *ranking)
        })();

        match ranking_data {
            Some(ref ranking) if ranking.state() == crate::ir::ranking_data::FINISH => {
                if ranking.total_player() > 0 {
            ...
            _ => {
                ...
                if !self.loading_initiated
                    && let Some(conn_arc) = main.ir_connection().cloned()
    ...
    fn target_rank(
        &self,
        main: &MainController,
        ranking: &crate::ir::ranking_data::RankingData,
    ) -> i32 {
        let total = ranking.total_player();
...
mod tests {
    ...
    struct MockIRConnection {
        scores: Vec<crate::ir::ir_score_data::IRScoreData>,
    }
    ...
    impl crate::ir::ir_connection::IRConnection for MockIRConnection {
        ...
        {
            crate::ir::ir_response::IRResponse::success("OK".to_string(), vec![])
        }
        fn get_table_datas(
            &self,
        ...
        {
            crate::ir::ir_response::IRResponse::success("OK".to_string(), self.scores.clone())
        }
        ...
        fn send_play_data(
            ...
            crate::ir::ir_response::IRResponse::success("OK".to_string(), ())
        }
    ...
    fn test_ir_async_load_success() {
        ...
        let ir_score = crate::ir::ir_score_data::IRScoreData::new(&score_data);

        let conn: Arc<dyn crate::ir::ir_connection::IRConnection + Send + Sync> =
            Arc::new(MockIRConnection {
                scores: vec![ir_score],
            });
        let chart = crate::ir::ir_chart_data::IRChartData::default();
    ...
    fn test_ir_rankrate_100_percent_does_not_oob() {
        ...
        let scores: Vec<crate::ir::ir_score_data::IRScoreData> = (0..5)
            .map(|i| {
                let mut sd = ScoreData::default();
                sd.player = format!("Player{}", i);
                sd.judge_counts.epg = 100 - i * 10;
                crate::ir::ir_score_data::IRScoreData::new(&sd)
            })
            .collect();
    ...
    fn test_ir_next_target_rank_fallback_sync_path() {
        ...
        assert_eq!(ranking.state(), crate::ir::ranking_data::FINISH);

...
impl BMSPlayer {
    ...
    pub(super) fn input_impl(&mut self) {
            ...
            (self.input.control.take(), self.lanerender.as_mut())
        {
            ...
            let mut ctx = crate::play::control_input_processor::ControlInputContext {
                lanerender,
        ...
        let is_autoplay = self.play_mode.mode == crate::core::bms_player_mode::Mode::Autoplay;
        if let Some(ref mut keyinput) = self.input.keyinput {
            let mut ctx = crate::play::key_input_processor::InputContext {
                now,
                key_states: &self.input.input_key_states,

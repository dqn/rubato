...
mod prop_tests {
    ...
    proptest! {
        ...
        #[test]
        fn next_int_bounded_in_range(seed: i64, bound in 1..=1000i32) {
            let mut rng = JavaRandom::new(seed);
            for _ in 0..20 {
                let val = rng.next_int_bounded(bound);
                prop_assert!(val >= 0 && val < bound, "val={} not in [0, {})", val, bound);
            }
        }

        #[test]
        fn deterministic_same_seed(seed: i64, bound in 1..=1000i32) {
            let mut rng1 = JavaRandom::new(seed);
            let mut rng2 = JavaRandom::new(seed);
            for _ in 0..10 {
                prop_assert_eq!(rng1.next_int_bounded(bound), rng2.next_int_bounded(bound));
            }
        }

        #[test]
        fn set_seed_resets_sequence(seed: i64, bound in 1..=1000i32) {
            let mut rng = JavaRandom::new(seed);
            let first_seq: Vec<i32> = (0..10).map(|_| rng.next_int_bounded(bound)).collect();
            rng.set_seed(seed);
            let second_seq: Vec<i32> = (0..10).map(|_| rng.next_int_bounded(bound)).collect();
            prop_assert_eq!(first_seq, second_seq);
        }

        #[test]
        fn power_of_two_bound_in_range(seed: i64, exp in 0..=10u32) {
            let bound = 1i32 << exp; // 1, 2, 4, ..., 1024
            let mut rng = JavaRandom::new(seed);
            for _ in 0..20 {
                let val = rng.next_int_bounded(bound);
                prop_assert!(val >= 0 && val < bound, "val={} not in [0, {})", val, bound);
        ...
        #[test]
        fn next_double_in_range(seed: i64) {
            let mut rng = JavaRandom::new(seed);
            for _ in 0..20 {
                let val = rng.next_double();

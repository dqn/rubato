...
impl SelectSkinContext<'_> {
    ...
    fn selected_directory_data(
        &self,
    ) -> Option<&crate::state::select::bar::directory_bar::DirectoryBarData> {
    ...
    fn selected_score_property(
        &self,
    ) -> Option<crate::core::score_data_property::ScoreDataProperty> {
        let score = self.selected_score()?;
        let mut property = crate::core::score_data_property::ScoreDataProperty::new();
    ...
    fn player_data(&self) -> Option<&rubato_types::player_data::PlayerData> {
        self.selector
            .player_resource
            .as_ref()
            .map(crate::core::player_resource::PlayerResource::player_data)
    }
...
impl rubato_types::skin_render_context::SkinRenderContext for SelectSkinContext<'_> {
    ...
    fn integer_value(&self, id: i32) -> i32 {
        match id {
            ...
            31 => self.player_data().map_or(0, |data| data.clear as i32),
            32 => self
                .player_data()
                .map_or(0, |data| (data.playcount - data.clear) as i32),
            33 => self
                .player_data()
                .map_or(0, |data| data.judge_count(0) as i32),
            34 => self
                .player_data()
                .map_or(0, |data| data.judge_count(1) as i32),
            35 => self
                .player_data()
                .map_or(0, |data| data.judge_count(2) as i32),
            36 => self
                .player_data()
                .map_or(0, |data| data.judge_count(3) as i32),
            37 => self
                .player_data()
                .map_or(0, |data| data.judge_count(4) as i32),
    ...
    fn string_value(&self, id: i32) -> String {
        match id {
            ...
            // Version
            1010 => crate::core::version::Version::get_version().to_string(),
            // Song hash (MD5)
            1030 => self
                .selected_song_data()
...
pub struct MusicSelector {
    ...
    player_resource: Option<crate::core::player_resource::PlayerResource>,

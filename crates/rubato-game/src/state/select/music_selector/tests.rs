...
use crate::state::select::bar::bar::Bar;
...
fn test_select_directory_bar() {
    ...
    let bar = Bar::Folder(Box::new(
        crate::state::select::bar::folder_bar::FolderBar::new(None, "test_crc".to_string()),
    ));
...
fn test_ranking_position_with_ir() {
    ...
    use crate::core::score_data::ScoreData as CoreScoreData;
...
fn test_render_skin_draws_ecfn_songlist_bitmap_bartext_quads() {
    ...
    let (manual_bitmap_quads, manual_textured_quads) = {
        ...
        let prepare_ctx = crate::state::select::bar_renderer::PrepareContext {
        ...
        let render_ctx = crate::state::select::bar_renderer::RenderContext {
            center_bar: selector.bar_rendering.select_center_bar,
            currentsongs: &selector.manager.currentsongs,
            rival: false,
            state: &adapter,
...
fn test_process_input_with_context_basic() {
    use crate::state::select::music_select_input_processor::MusicSelectInputProcessor;
...
fn test_read_random_course_missing_songs_does_not_transition() {
    ...
    selector.manager.currentsongs = vec![Bar::RandomCourse(Box::new(
        crate::state::select::bar::random_course_bar::RandomCourseBar::new(rcd),
    ))];
    selector.manager.selectedindex = 0;
...
fn test_directory_autoplay_path_extraction_from_container_bar() {
    ...
    let container =
        crate::state::select::bar::container_bar::ContainerBar::new(String::new(), children);
...
fn make_ranking_data_with_scores() -> crate::ir::ranking_data::RankingData {
    use crate::core::clear_type::ClearType;
    use crate::ir::ir_score_data::IRScoreData;

    let mut rd = crate::ir::ranking_data::RankingData::new();
...
fn make_ir_ranking_data(exscores: &[i32]) -> RankingData {
    ...
    let scores: Vec<IRScoreData> = exscores
        ...
        .map(|(i, &ex)| {
            ...
            s.clear = crate::core::clear_type::ClearType::Normal.id();
            IRScoreData::new(&s)
        })
        .collect();
    rd.update_score(&scores, None);
...
#[test]
fn set_float_value_volume_propagates_to_pending_audio_config() {
    ...
    // Verify the audio config is queued in the outbox for MainController to consume.
    let pending = selector
        .pending_audio_config
        .as_ref()
        .expect("pending_audio_config should be set");
    assert_eq!(pending.systemvolume, 0.75);
    assert_eq!(pending.keyvolume, 0.5);
    assert_eq!(pending.bgvolume, 0.25);
}

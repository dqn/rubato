...
use crate::core::pixmap_resource_pool::PixmapResourcePool;
...
pub struct LoaderContext<'a> {
    ...
    pub song_info_db: Option<&'a dyn crate::song_information_db::SongInformationDb>,
    /// CommandBar context for folder status updates (score/scorelog paths)
    pub command_bar_ctx: Option<crate::state::select::bar::command_bar::CommandBarContext<'a>>,
}
            ...
            if let Some(ref ctx) = ctx {
                ...
                match bar {
                    ...
                    Bar::Command(b) => {
                        ...
                        let cmd_ctx = crate::state::select::bar::command_bar::CommandBarContext {
        ...
        if !l.is_empty() {
            ...
            if sourcebar_title.is_some() {
                ...
                if prevbar_is_song && prevbar_sha256.is_some() {
                    ...
                    if let Some(pos) = self.currentsongs.iter().position(|bar| {
                        bar.as_song_bar()
                            .is_some_and(|sb| sb.exists_song() && sb.song_data().file.sha256 == sha)
                    }) {
                        self.selectedindex = pos;
                    }

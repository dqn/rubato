...
mod tests {
    ...
    fn test_bar_type_classify_song() {
        ...
        let bar = Bar::Song(Box::new(crate::state::select::bar::song_bar::SongBar::new(
            song_data,
        )));
    ...
    fn test_bar_type_classify_directory() {
        let bar = Bar::Folder(Box::new(
            crate::state::select::bar::folder_bar::FolderBar::new(None, "crc".to_string()),
        ));
    ...
    fn test_bar_type_classify_function() {
        let bar = Bar::Function(Box::new(
            crate::state::select::bar::function_bar::FunctionBar::new("test".to_string(), 0),
        ));

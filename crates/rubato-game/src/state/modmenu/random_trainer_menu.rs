            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    for lane in lane_order.iter() {
                        ...
                        let is_random =
                            crate::state::modmenu::random_trainer::RandomTrainer::is_lane_to_random(
                                lane_char,
                            );
                ...
                crate::state::modmenu::random_trainer::RandomTrainer::set_active(trainer_enabled);
                if trainer_enabled {
                    let current = get_lane_order_string();
                    let trainer =
                        crate::state::modmenu::random_trainer::RandomTrainer::get_current_lane_order();
                    if current != trainer {
                        crate::state::modmenu::random_trainer::RandomTrainer::set_lane_order(&current);
                    }
                }

                let bw = *lock_or_recover(&BLACK_WHITE_RANDOM_PERMUTATION);
                crate::state::modmenu::random_trainer::RandomTrainer::set_black_white_permute(bw);
            });
    }
}

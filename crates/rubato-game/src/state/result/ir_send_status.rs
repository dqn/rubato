...
pub struct IRSendStatusMain {
    ...
    pub score: crate::core::score_data::ScoreData,
    pub retry: i32,
    pub last_try: i64,
    pub is_sent: bool,
}
...
impl IRSendStatusMain {
    pub fn new(
        ...
        songdata: &SongData,
        score: &crate::core::score_data::ScoreData,
    ) -> Self {
        Self {
            connection,
    ...
    pub fn send(&mut self) -> bool {
        log::info!("IRへスコア送信中 : {}", self.songdata.metadata.title);
        ...
        if send1.is_succeeded() {
            log::info!("IRスコア送信完了 : {}", self.songdata.metadata.title);
            self.is_sent = true;
            true
        } else {
...
mod tests {
    ...
    fn make_test_song() -> SongData {
        ...
        song.metadata.subtitle = "Test Sub".to_string();
        song
    }

    fn make_test_score() -> crate::core::score_data::ScoreData {
        crate::core::score_data::ScoreData::default()
    }

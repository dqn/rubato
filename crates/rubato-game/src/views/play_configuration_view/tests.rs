...
fn test_update_delegates_to_video_controller() {
    ...
    let config = Config {
        display: crate::core::config::DisplayConfig {
            vsync: true,
...
fn test_update_delegates_to_music_select_controller() {
    let mut view = initialized_view();
    let config = Config {
        select: crate::core::config::SelectConfig {
...
fn test_update_delegates_to_discord_controller() {
    let mut view = initialized_view();
    let config = Config {
        integration: crate::core::config::IntegrationConfig {
...
fn test_update_delegates_to_obs_controller() {
    let mut view = initialized_view();
    let config = Config {
        obs: crate::core::config::ObsConfig {
            use_obs_ws: true,
...
fn test_update_commit_roundtrip_preserves_config_fields() {
    ...
    let config = Config {
        ...
        network: crate::core::config::NetworkConfig {
            enable_ipfs: true,
...
fn test_import_score_data_from_lr2_imports_matching_scores() {
    ...
    let scoredb = crate::core::score_database_accessor::ScoreDatabaseAccessor::new(
...
fn test_commit_player_copies_back_ir_controller_mutations() {
    ...
    let stale_ir = crate::core::ir_config::IRConfig {

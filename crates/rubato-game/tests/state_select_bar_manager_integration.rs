...
use rubato_game::state::select::bar::hash_bar::HashBar;
use rubato_game::state::select::bar::search_word_bar::SearchWordBar;
...
fn update_bar_root_clears_directory_stack() {
    ...
    manager.dir.push(Box::new(Bar::Folder(Box::new(
        rubato_game::state::select::bar::folder_bar::FolderBar::new(None, "test_dir".to_string()),
    ))));
...
fn close_returns_to_parent() {
    ...
    manager.dir.push(Box::new(Bar::Folder(Box::new(
        rubato_game::state::select::bar::folder_bar::FolderBar::new(None, "parent".to_string()),
    ))));

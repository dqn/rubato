...
mod prop_tests {
    ...
    proptest! {
        ...
        #[test]
        fn next_int_in_range(seed: i32, max in 1..=1000i32) {
            let mut rng = LR2Random::with_seed(seed);
            for _ in 0..20 {
                let val = rng.next_int(max);
                prop_assert!(val >= 0 && val < max, "val={} not in [0, {})", val, max);
            }
        }

        #[test]
        fn deterministic_same_seed(seed: i32, max in 1..=1000i32) {
            let mut rng1 = LR2Random::with_seed(seed);
            let mut rng2 = LR2Random::with_seed(seed);
            for _ in 0..10 {
                prop_assert_eq!(rng1.next_int(max), rng2.next_int(max));
            }
        }

        #[test]
        fn set_seed_resets_state(seed: i32, max in 1..=1000i32) {
            let mut rng = LR2Random::with_seed(seed);
            let first_seq: Vec<i32> = (0..10).map(|_| rng.next_int(max)).collect();
            rng.set_seed(seed);
            let second_seq: Vec<i32> = (0..10).map(|_| rng.next_int(max)).collect();

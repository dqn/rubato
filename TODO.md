...
- [ ] beatoraja-select `NullSongDatabaseAccessor` is a full trait impl in stubs.rs — could be moved to proper module (used by music_selector.rs). Low priority organizational refactoring
...
- [ ] All remaining stubs (16 files, ~2,700 lines) thoroughly audited — 97% are correctly stubbed or intentionally placed. No further easy wins. Remaining blockers: rendering pipeline, IR/network layer, database queries, per-screen MainState implementations
...
- **beatoraja-select remaining stubs:** MainState trait (empty marker), RandomCourseData/RandomStageData (blocked: need random course generation logic), RankingData/RankingDataCache (blocked: need IR integration), SkinHeader (empty struct, real type in beatoraja-skin), SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering pipeline), EventType (used by music_selector execute_event), AudioDriver (used by preview_music_processor), NullSongDatabaseAccessor (used by music_selector, could move to proper module), DownloadTask/DownloadTaskState/DownloadTaskStatus (used by download_task_menu). BMSPlayerMode/PlayerInformation replaced (18e-7). Dead Pair removed (18e-7). SongManagerMenu delegates to beatoraja-types (18e-8). BMSPlayerInputProcessor/ControlKeys/KeyCommand replaced (18e-9). TimerState removed as dead code (18e-9)

...
#### 18e-8: Circular dependency resolution (complete)

- [x] Move RandomHistoryEntry + history state to beatoraja-types — created `random_history` module with `RandomHistoryEntry` struct, `LANE_ORDER_HISTORY` static, `add_random_history()`/`get_random_history()` free functions. Breaks modmenu→pattern circular dep
- [x] Update beatoraja-pattern to use beatoraja-types — added beatoraja-types dependency, replaced stub usage in lane_shuffle_modifier.rs with `add_random_history(RandomHistoryEntry::new(...))`. Deleted beatoraja-pattern/src/stubs.rs entirely (was only RandomTrainer). Removed `pub mod stubs;` from lib.rs
- [x] Update beatoraja-modmenu RandomTrainer to delegate to beatoraja-types — removed local `RandomHistoryEntry` struct + `LANE_ORDER_HISTORY` static, re-exports `RandomHistoryEntry` from beatoraja-types, `add_random_history()`/`get_random_history()` delegate to shared module
- [x] Move last-played-sort state to beatoraja-types — created `last_played_sort` module with `LAST_PLAYED_SORT` static, `is_enabled()`/`force_disable()`/`set()` functions. Breaks modmenu→select circular dep
- [x] Update beatoraja-select SongManagerMenu to delegate to beatoraja-types — stub now delegates to `beatoraja_types::last_played_sort` instead of returning hardcoded false/no-op
- [x] Update beatoraja-modmenu SongManagerMenu to use shared state — removed local `LAST_PLAYED_SORT` static, all methods delegate to `beatoraja_types::last_played_sort`, egui checkbox uses get/set pattern

#### 18e-9: BMSPlayerInputProcessor replacement + beatoraja-select input stubs + IR module extraction (complete)

- [x] Replace BMSPlayerInputProcessor stub in beatoraja-result with `pub use beatoraja_input::bms_player_input_processor::BMSPlayerInputProcessor` — removed ~31 lines of no-op stub code. Real type has `&mut self` on `is_control_key_pressed`/`is_activated` (stub had `&self`), compatible because callers get `&mut` from `main.get_input_processor()`. MainController stub updated to create real instance via `BMSPlayerInputProcessor::new(&Config::default(), &PlayerConfig::default())`
- [x] Uncomment `input.reset_scroll()` in abstract_result.rs — was commented out as "would need &mut, but this is a stub". Real type supports `&mut self` via `get_input_processor()`
- [x] Replace BMSPlayerInputProcessor/ControlKeys/KeyCommand stubs in beatoraja-select with `pub use` from beatoraja-input — removed ~72 lines of stub code. 3 methods in music_select_key_property.rs updated from `&BMSPlayerInputProcessor` to `&mut BMSPlayerInputProcessor` (real type has `&mut self` on `reset_key_changed_time`/`get_analog_diff_and_reset`)
- [x] Move IRStatus and IRSendStatusMain from beatoraja-result stubs.rs to dedicated modules — `ir_status.rs` and `ir_send_status.rs`. Re-exports in stubs.rs keep callers unchanged. Purely organizational — these were real types incorrectly placed in stubs.rs
- [x] Remove dead imports from beatoraja-result stubs.rs — `use std::sync::Arc` and `use beatoraja_types::song_data::SongData` unused after IR types moved to own modules
- [x] Remove dead `TimerState` struct from beatoraja-select stubs.rs — defined but never used outside stubs.rs (~20 lines removed)

#### 18e-10: SkinHeader + RandomCourseData replacement, NullSongDatabaseAccessor module extraction (complete)

- [x] Replace empty `SkinHeader` stub in beatoraja-select with `pub use beatoraja_skin::skin_header::SkinHeader` — real type already used by beatoraja-result and beatoraja-modmenu. Dependency `beatoraja-skin` was already present. Only used as field type in `MusicSelectSkin.header` (never constructed directly). Zero caller changes needed
- [x] Move `NullSongDatabaseAccessor` from stubs.rs to `crate::null_song_database_accessor` module — full `SongDatabaseAccessor` trait impl with log warnings. Re-export in stubs.rs preserves backward compatibility. Used by `MusicSelector::new()` as default database accessor
- [x] Replace `RandomCourseData` stub in beatoraja-select with `pub use beatoraja_core::random_course_data::RandomCourseData` — added Java-style getters (`get_name`, `get_stage`, `get_song_datas`, `EMPTY` const) to real type for caller compatibility. Real type has full `create_course_data()` (with timestamp) and `lottery_song_data()` (with duplicate avoidance). ~30 lines of stub code removed
- [x] Replace `RandomStageData` unit struct stub with `pub use beatoraja_core::random_stage_data::RandomStageData` — real type has `title: Option<String>` and `sql: Option<String>` fields
...
#### 18e-11: Cross-crate stub replacements (complete)

- [x] Replace `RankingData` stub in beatoraja-select with `pub use beatoraja_ir::ranking_data::RankingData` — fixes latent bug (stub constants ACCESS=0/FINISH=1/FAIL=2 were off by 1 from Java; real type has correct NONE=0/ACCESS=1/FINISH=2/FAIL=3). Real type has full IR ranking with `update_score()`, `get_rank()`, `get_previous_rank()`, `get_local_rank()`, `get_score()`, `get_clear_count()`. ~42 lines of stub code removed
- [x] Replace `RankingDataCache` stub in beatoraja-select with `pub use beatoraja_ir::ranking_data_cache::RankingDataCache` — real type has HashMap-based caching with SHA-256 course hash. ~17 lines of stub code removed. Was only used within stubs.rs (no external callers)
- [x] Replace `InputProcessorStub`/`ControlKeysStub` in beatoraja-decide with real `BMSPlayerInputProcessor`/`ControlKeys` from beatoraja-input — `MainControllerRef::get_input_processor` changed from `&self -> &InputProcessorStub` to `&mut self -> &mut BMSPlayerInputProcessor` (Box::leak pattern). music_decide.rs `input()` restructured to avoid overlapping `&mut` borrows (scoped block collects booleans before `get_audio_processor` call). ~26 lines of stub code removed
- [x] Replace `TableData`/`TableFolder`/`TableDataAccessor`/`TableAccessor` stubs in beatoraja-external with `pub use` from beatoraja-core — `BMSSearchAccessor::write` signature updated from `&TableData` to `&mut TableData` to match real `TableAccessor` trait. ~74 lines of stub code removed
...
- [x] beatoraja-result stubs.rs: `Arc` and `SongData` imports became unused after IRStatus/IRSendStatusMain moved to own modules. Cleaned up in 18e-9
- [x] beatoraja-select stubs.rs: `TimerState` was dead code (defined but never used outside stubs.rs). Removed in 18e-9
- [x] beatoraja-select `NullSongDatabaseAccessor` moved from stubs.rs to dedicated module `null_song_database_accessor.rs` — re-export in stubs.rs preserves backward compatibility. Done in Phase 18e-10
- [ ] beatoraja-external LibGDX stubs (Pixmap/GdxGraphics/BufferUtils/PixmapIO) are actively used by screenshot exporters — blocked on wgpu rendering pipeline for real implementation
- [x] RankingData constant mismatch resolved — stub had ACCESS=0/FINISH=1/FAIL=2 (off by 1 from Java), replaced with real type in Phase 18e-11
- [x] beatoraja-decide InputProcessorStub/ControlKeysStub replaced with real types from beatoraja-input — Box::leak pattern for `&mut` return, borrow-conflict fix in music_decide.rs. Done in Phase 18e-11
- [x] beatoraja-external TableData/TableFolder/TableDataAccessor/TableAccessor replaced with real types from beatoraja-core — `write(&self, td: &TableData)` → `write(&self, td: &mut TableData)`. Done in Phase 18e-11
- [ ] All remaining stubs (16 files, ~2,440 lines) exhaustively audited across 4 rounds — all actionable replacements completed. Remaining stubs (rendering, IR network, database, per-screen implementations) are genuinely blocked. beatoraja-obs/stream/ir/md-processor stubs are clean (re-exports only). beatoraja-decide: MainControllerRef (change_state), AudioProcessorStub, SkinStub (blocked: real MainController). beatoraja-external: LibGDX, Twitter4j, Property traits, ScoreDatabaseAccessor, MainState/MainStateListener/AbstractResult/ScreenType (blocked: rendering + per-screen types)

## Remaining Stubs

- **Lifecycle (mostly resolved):** MainController stubs removed from obs/external/ir/select/modmenu (Phase 18e-2). md-processor reclassified as intentional adapter pattern. modmenu: dead code removed, skin config types replaced with real beatoraja-types, skin stubs replaced with real beatoraja-skin types (Phase 18e-3); 3-method MainController stub retained for lifecycle. Remaining MainController: result (6 methods actively used, 3 unused pruned, PlayDataAccessor resolved in 18e-4, BMSPlayerInputProcessor resolved in 18e-9, remaining blocker: IRConnection not implemented). **PlayerResource wrapper migration complete for all 6 crates** — select/decide/external use `Box<dyn PlayerResourceAccess>`, result uses wrapper struct with crate-local fields for non-trait types. MainState stubs require per-screen concrete implementations
- **beatoraja-select remaining stubs:** MainState trait (empty marker), SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering pipeline), EventType (used by music_selector execute_event stubs), AudioDriver (used by preview_music_processor), DownloadTask/DownloadTaskState/DownloadTaskStatus (used by download_task_menu). RankingData/RankingDataCache replaced with real beatoraja-ir types (18e-11). BMSPlayerMode/PlayerInformation replaced (18e-7). Dead Pair removed (18e-7). SongManagerMenu delegates to beatoraja-types (18e-8). BMSPlayerInputProcessor/ControlKeys/KeyCommand replaced (18e-9). TimerState removed as dead code (18e-9). SkinHeader replaced with real type (18e-10). NullSongDatabaseAccessor moved to own module (18e-10). RandomCourseData/RandomStageData replaced with real types (18e-10)
- **beatoraja-modmenu remaining stubs:** Skin/SkinObject/SkinObjectDestination/Rectangle (real SkinObject is incompatible enum), MainState trait, MainController (3 methods), MusicSelector/Bar/SongBar (select screen types). ImBoolean/ImInt/ImFloat replaced with plain primitives (18e-6). Clipboard replaced with arboard (18e-6). RandomTrainer delegates to beatoraja-types for history (18e-8). SongManagerMenu delegates to beatoraja-types for sort state (18e-8)
- **beatoraja-pattern:** stubs.rs deleted entirely (18e-8). RandomHistoryEntry + history state moved to beatoraja-types::random_history. Zero remaining stubs
- **Rendering re-exports:** `rendering_stubs.rs` in beatoraja-skin now re-exports real beatoraja-render types (resolved, not stubs)
- **beatoraja-decide remaining stubs:** MainControllerRef (change_state, get_audio_processor), AudioProcessorStub, SkinStub/load_skin/play_sound (blocked: real MainController + rendering). InputProcessorStub/ControlKeysStub replaced with real types (18e-11)
- **beatoraja-external remaining stubs:** LibGDX (Pixmap/GdxGraphics/BufferUtils/PixmapIO), Twitter4j, ClipboardHelper (image), Property traits (IntegerProperty/BooleanProperty/StringProperty + factories), ScoreDatabaseAccessor, MainState/MainStateListener, AbstractResult/ScreenType (blocked: rendering, per-screen types, incompatible APIs). TableData/TableFolder/TableDataAccessor/TableAccessor replaced with real types (18e-11)
- **beatoraja-result remaining stubs:** MainController (6 methods, PlayDataAccessor resolved, BMSPlayerInputProcessor resolved in 18e-9), SkinObjectData (minimal, real type incompatible — 20+ fields). IRStatus/IRSendStatusMain moved to own modules (18e-9, send() still stubbed). freq_trainer stubs replaced with re-exports from beatoraja-modmenu (Phase 18e-6). BMSPlayerMode/BMSPlayerModeType replaced with real types (Phase 18e-5), EventType removed (dead code). PlayerResource wrapper complete; non-trait field types (BMSModel, RankingData) stored locally, BMSPlayerMode now uses real type

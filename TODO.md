...
- [ ] beatoraja-result: Stub type conversion complete (FloatArray→Vec<f32>, GdxArray→Vec, GrooveGaugeStub→real GrooveGauge). PlayerResource stub methods now match trait signatures for gauge types. Remaining blockers: types not on trait (`BMSModel`, `RankingData`, `BMSPlayerMode`), `get_replay_data_mut`/`get_course_score_data_mut` (needed in stub but not actually called — can be removed). Next step: create wrapper struct with `Box<dyn PlayerResourceAccess>` + non-trait fields (BMSModel, BMSPlayerMode, RankingData), then update callers
...
- [x] beatoraja-result stub type conversion complete — `FloatArray` → `Vec<f32>`, `GdxArray<T>` → `Vec<T>`, `GrooveGaugeStub` → real `GrooveGauge` (from beatoraja-types). Callers in music_result.rs, course_result.rs, skin_gauge_graph_object.rs updated. Stub methods `get_gauge()`, `get_groove_gauge()`, `get_course_gauge()`, `get_course_gauge_mut()`, `add_course_gauge()` now match trait signatures. `IntArray` retained (only used in skin_gauge_graph_object.rs). PlayerResource → `Box<dyn PlayerResourceAccess>` wrapper migration now unblocked
- [ ] beatoraja-result needs wrapper for non-trait types — `BMSModel` (bms-model), `BMSPlayerMode` (local stub), `RankingData` (beatoraja-ir) cannot be on `PlayerResourceAccess` trait (circular deps). Use wrapper struct pattern (like beatoraja-external) with `Box<dyn PlayerResourceAccess>` + extra fields

## Remaining Stubs

- **Lifecycle (mostly resolved):** MainController stubs removed from obs/external/ir/select/modmenu (Phase 18e-2). md-processor reclassified as intentional adapter pattern. modmenu: dead code removed, skin config types replaced with real beatoraja-types; 3-method MainController stub retained for lifecycle. Remaining MainController: result (6 methods actively used, 3 unused pruned, blocked on type mismatches). PlayerResource stubs removed from select/decide/external. Remaining PlayerResource: result (stub type conversion done — FloatArray/GdxArray/GrooveGaugeStub removed, gauge methods match trait signatures; blocked on wrapper struct for non-trait types: BMSModel, BMSPlayerMode, RankingData). MainState stubs require per-screen concrete implementations

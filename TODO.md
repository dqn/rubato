...
- [ ] All remaining stubs (16 files, ~2,600 lines) exhaustively audited across 3 rounds — all actionable replacements completed. Remaining stubs (rendering, IR, database, per-screen implementations) are genuinely blocked. beatoraja-obs/stream/ir/md-processor stubs are clean (re-exports only). beatoraja-decide: MainControllerRef + input/audio/skin stubs (blocked: real MainController). beatoraja-external: LibGDX, Twitter4j, Property traits, TableData API mismatch (blocked: rendering + incompatible APIs). RankingData constants: stub ACCESS=0 vs real ACCESS=1 (value mismatch prevents direct replacement)
...
- **beatoraja-select remaining stubs:** MainState trait (empty marker), RankingData/RankingDataCache (blocked: constant value mismatch with beatoraja-ir real type — stub ACCESS=0 vs real ACCESS=1), SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering pipeline), EventType (used by music_selector execute_event stubs), AudioDriver (used by preview_music_processor), DownloadTask/DownloadTaskState/DownloadTaskStatus (used by download_task_menu). BMSPlayerMode/PlayerInformation replaced (18e-7). Dead Pair removed (18e-7). SongManagerMenu delegates to beatoraja-types (18e-8). BMSPlayerInputProcessor/ControlKeys/KeyCommand replaced (18e-9). TimerState removed as dead code (18e-9). SkinHeader replaced with real type (18e-10). NullSongDatabaseAccessor moved to own module (18e-10). RandomCourseData/RandomStageData replaced with real types (18e-10)

...
All phases (1–25d) complete. **1739 tests pass, 22 ignored.** See AGENTS.md for full status.
...
#### 25d-2: レンダリング連携スタブの解消 — complete

5 新規型を beatoraja-types に追加 (SkinOffset, TimerAccess, TimingDistribution, EventType, SkinMainState)。beatoraja-skin/select/modmenu で ~88行スタブ削除。+9テスト。

残存 (API 非互換のためリファクタリング必要 → **Phase 29a**):
- beatoraja-select: SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (simplified API vs real)
- beatoraja-modmenu: Skin/SkinObject (real = complex struct/enum)、MusicSelector/Bar (not Send+Sync)
- beatoraja-select: DownloadTask (needs md-processor dep)
...
| 25d | Stub resolution: 9 types/traits in beatoraja-types, ~620 stub lines eliminated | +38 |
